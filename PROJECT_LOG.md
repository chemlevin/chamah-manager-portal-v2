...
- Added no-cache/revalidation headers for HTML, JavaScript, CSS, and the service-worker script. The service-worker script also has `no-store`, and the same behavior is represented by the local static server for regression testing.
