...
## TRACK053 — Upgrade cmh-ops to Node 24 (2026-09-24)

- Confirmed Vercel project `cmh-ops` (`prj_GBZ13QkCzik7cHYDqKRFDLBDUUwT`) was configured for Node.js `20.x`, then updated only that project setting to Node.js `24.x`. The separate `chamah-portal` project was not reconfigured or deployed.
- The shared root `package.json` does not pin a Node version, so no package or lockfile change was required. Adding a shared engine constraint solely for `cmh-ops` would have affected the unrelated Vercel project and was intentionally avoided.
- Local compatibility validation used Node `v24.18.0`: `npm run build` passed, and the initially flaky shared-autosave case passed on focused rerun (4/4 tests). The broad `@playwright/test` run was not a clean regression gate because browser cases could not launch in the restricted local environment; the 976-case serial run was stopped after 198 cases rather than misreported as passing. The remote Vercel build is the authoritative runtime-compatibility check for this configuration-only TRACK.
- Redeployed the existing TRACK052 Preview artifact for `cmh-ops` after the runtime change. Preview deployment `dpl_AHcJLMPshNDuTwRQJ8qCy6PDW6UT` is READY at `https://cmh-1wzu7kwi3-chamah.vercel.app`; Vercel installed dependencies, ran the project build, and deployed all three Node functions successfully. The source artifact remains commit `18eae62939a2457bce4d61043a825520f5743fd4`.
- Production was not deployed or promoted. The existing `cmh-ops` Production artifact remains unchanged; future `cmh-ops` builds use Node.js 24.x.

## TRACK055 — Deploy cmh-ops Node 24 (2026-09-24)

- Verified the target before promotion as Vercel project `cmh-ops` (`prj_GBZ13QkCzik7cHYDqKRFDLBDUUwT`) in team `chamah`, configured with `nodeVersion: 24.x`. The validated TRACK053 Preview was `dpl_AHcJLMPshNDuTwRQJ8qCy6PDW6UT`, READY in that same project and sourced from Git SHA `18eae62939a2457bce4d61043a825520f5743fd4`.
- Promoted only that Preview artifact to READY Production deployment `dpl_9kV85MU1AweJUFm3QJXiiWBBgawo`. Existing Production aliases `https://cmh-ops-chamah.vercel.app` and `https://chamah-manager-portal-v2.vercel.app` independently resolved to the new deployment and serving SHA.
- Production deployment metadata reports `nodeVersion: 24.x`, the original TRACK053 deployment ID, and all three Node functions. Authenticated read-only smoke requests returned HTTP 200 for `/api/budget-test`, `/api/budget`, and `/api/allocations`; `/api/budget-test` returned `{ ok: true, route: "budget-test" }`.
- No `chamah-portal` deployment, database, migration, Supabase data/configuration, Edge Function, application source, API contract, calculation, package, lockfile, domain, or unrelated project was changed.
