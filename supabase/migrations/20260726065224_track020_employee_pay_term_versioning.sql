-- TRACK020 completion: employee manager relationship and immutable pay-term versions.
...
begin
  ...
  where employee_id = target_employee_id
    and valid_from <= effective_from
    and (valid_to is null or valid_to >= effective_from)
  order by valid_from desc
  limit 1
  for update;

  if previous_term.employee_pay_term_id is not null
     and previous_term.valid_from = effective_from then
  ...
  insert into public.employee_pay_terms (
    ...
    nullif(term_values->>'pay_type', ''),
    nullif(term_values->>'base_pay', '')::numeric,
    nullif(term_values->>'estimated_employment_percentage', '')::numeric,
    nullif(term_values->>'notes', ''),
    actor_id,
    actor_id
  )
...
create or replace function public.portal_close_employee_pay_term(
  target_pay_term_id uuid,
  close_on date,
  actor_id uuid
)
returns jsonb
...
declare
  ...
  saved_term public.employee_pay_terms;
begin
  if target_pay_term_id is null or close_on is null or actor_id is null then
    raise exception 'Pay term, closing date and user are required';
  end if;

  select * into previous_term
  from public.employee_pay_terms
  where employee_pay_term_id = target_pay_term_id
  for update;
  if not found then raise exception 'Pay term not found'; end if;
  if close_on < previous_term.valid_from then
    raise exception 'Closing date cannot precede the version start date';
  end if;
  if previous_term.valid_to is not null and previous_term.valid_to < current_date then
    raise exception 'Expired pay-term history cannot be changed';
  end if;

  update public.employee_pay_terms
  set valid_to = close_on,
      updated_by_user_id = actor_id
  where employee_pay_term_id = target_pay_term_id
  returning * into saved_term;
  ...
  insert into public.audit_events (
    ...
    'employee_pay_terms', saved_term.employee_pay_term_id, 'CLOSE',
    to_jsonb(previous_term), to_jsonb(saved_term), 'PORTAL_ADMIN', actor_id
  );
...
revoke all on function public.portal_close_employee_pay_term(uuid, date, uuid) from public, anon, authenticated;
grant execute on function public.portal_version_employee_pay_term(uuid, date, jsonb, uuid) to service_role;
grant execute on function public.portal_close_employee_pay_term(uuid, date, uuid) to service_role;

comment on function public.portal_version_employee_pay_term(uuid, date, jsonb, uuid)
  is 'Creates a new effective-dated pay-term version and closes the overlapping version without overwriting history.';
