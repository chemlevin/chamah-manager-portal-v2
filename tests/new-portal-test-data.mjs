...
export const portalAccessFixture = { profile: { user_id: 'test-user', display_name: 'משתמשת בדיקה', is_active: true, is_super_admin: true, scope_mode: 'ORGANIZATION' }, allocation_unit_ids: [], daycare_ids: [], sections: [
  ['home', 'home', null], ['dashboards', 'dashboards', null], ['dashboards.finance', 'dashboards/unit/organization/finance', 'dashboards'], ['dashboards.accounting', 'dashboards/unit/organization/accounting', 'dashboards'], ['dashboards.accounting.summary', 'dashboards/unit/organization/accounting/summary', 'dashboards.accounting'], ['dashboards.accounting.banks', 'dashboards/unit/organization/accounting/banks', 'dashboards.accounting'], ['dashboards.accounting.bank-transfers', 'dashboards/unit/organization/accounting/bank-transfers', 'dashboards.accounting'], ['dashboards.licensing', 'dashboards/unit/organization/licensing', 'dashboards'], ['dashboards.team', 'dashboards/unit/organization/team', 'dashboards'], ['dashboards.staffing', 'dashboards/unit/organization/staffing', 'dashboards'], ['dashboards.staffing.employees', 'dashboards/unit/organization/staffing/employees', 'dashboards.staffing'], ['dashboards.staffing.employees.import', 'dashboards/unit/organization/staffing/employees/import', 'dashboards.staffing.employees'], ['dashboards.staffing.actual-payroll', 'dashboards/unit/organization/staffing/actual-payroll', 'dashboards.staffing'], ['dashboards.occupancy', 'dashboards/unit/organization/occupancy', 'dashboards'], ['calculators', 'calculators', null], ['calculators.salary', 'calculators/salary', 'calculators'], ['calculators.occupancy', 'calculators/occupancy', 'calculators'], ['payroll', 'payroll', null], ['payroll.open', 'payroll/open', 'payroll'], ['payroll.working', 'payroll/working', 'payroll'], ['payroll.closed', 'payroll/closed', 'payroll'], ['payroll.reports', 'payroll/reports', 'payroll'], ['management', 'training', null], ['management.permissions', 'training/permissions', 'management'], ['management.permissions.users', 'training/permissions/users', 'management.permissions'], ['management.rules', 'training/rules', 'management'], ['management.rules.calculation', 'training/rules/calculation', 'management.rules'], ['management.rules.system', 'training/rules/system', 'management.rules'], ['management.settings', 'training/settings', 'management'], ['management.tables', 'training/tables', 'management'], ['management.tables.calculation', 'training/tables/calculation', 'management.tables'], ['management.tables.variables', 'training/tables/variables', 'management.tables'], ['management.audit', 'training/audit', 'management'], ['knowledge', 'knowledge', null], ['maintenance', 'maintenance', null], ['tasks', 'tasks', null]
...
export async function mockNewPortalSupabase(page, units = allocationUnits) {
  ...
  await page.route('**/functions/v1/portal-workforce-workbench**', (route) => {
    ...
    const body = payroll ? {
      ...
      months: [
        { payroll_month_id: '44444444-4444-4444-8444-444444444444', payroll_month: '2026-07-01', month_status: 'CURRENT', scope_type: 'ORGANIZATION', opening_method: 'ACTIVE_EMPLOYEES' },
        { payroll_month_id: '55555555-5555-4555-8555-555555555555', payroll_month: '2026-06-01', month_status: 'CLOSED', scope_type: 'ORGANIZATION', opening_method: 'PREVIOUS_MONTH' },
      ],
      canReopen: true,
    } : {
      employees: generalResponses.employees.map((row) => ({ ...row, employee_code: 'EMP-1' })),
