...
export function mountEmployeeImport({request,getData,onImported,message}){
  ...
  const templateButton=document.querySelector("#employee-template"),importButton=document.querySelector("#employee-import");if(!templateButton||!importButton||!input||!dialog)return;templateButton.onclick=()=>downloadEmployeeTemplate();importButton.onclick=()=>{renderUpload();dialog.showModal();};input.onchange=()=>load(input.files[0]);
}
