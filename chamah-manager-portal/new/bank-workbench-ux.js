...
import { setLoadingFeedback, withBusyControl } from "./async-feedback.js";
...
export async function mountBankWorkbenchV2(request) {
  ...
  const renderWorkflowCards = () => {
    const counts=state.data.resultSummary||{total:state.data.pagination?.total||0,split:0,assigned:0,unassigned:0};
    const cards=[["total","רשומות"],["split","מפוצלות"],["assigned","שויכו"],["unassigned","לא שויכו"]];
    $("#bank-workflow-cards").innerHTML = cards.map(([key,title]) => `<article><span>${title}</span><strong>${counts[key]??0}</strong></article>`).join("");
  };
  const renderFilterChips = () => {
    const chips = [[null,`שנה: ${state.data.activeYear||state.year}`]];
    if (state.query) chips.push(["query",`חיפוש: ${state.query}`]);
    if (state.description) chips.push(["description",`תיאור: ${state.description}`]);
  ...
  const render = (preserve = true) => {
    ...
    $("#bank-load-progress").textContent=`נטענו ${rows.length} מתוך ${pagination.total??rows.length}`;
  ...
  const renderToolbarFilters = () => {
    ...
    $("#bank-workflow-filter").value=state.workflow;
  ...
  const requestPage = async (page) => {
    const params=new URLSearchParams({year:state.year,page:String(page),page_size:String(state.pageSize),query:state.query,description:state.description,queue:state.workflow,split:state.split,sort:state.sort,description_presence:state.descriptionPresence,reference_presence:state.referencePresence});
    ...
    return request("GET",undefined,params.toString());
  };
  const reload = async () => {
    const generation=++state.requestGeneration; state.page=1; state.loadingMore=false;
    const scroll=$("#bank-scroll"); scroll?.setAttribute("aria-busy","true");
    setLoadingFeedback($("#bank-load-more-status"),true,"טוען תנועות...");
    try {
      const response=await requestPage(1); if(generation!==state.requestGeneration)return;
      state.data={
      ...
      if(generation===state.requestGeneration){scroll?.setAttribute("aria-busy","false");setLoadingFeedback($("#bank-load-more-status"),false);}
    }
  };
  const loadMore = async () => {
    const pagination=state.data?.pagination;
    if(state.loadingMore||!pagination?.hasNext)return;
    const generation=state.requestGeneration, nextPage=(pagination.page||state.page)+1; state.loadingMore=true;
    $("#bank-scroll")?.setAttribute("aria-busy","true"); setLoadingFeedback($("#bank-load-more-status"),true,"טוען תנועות נוספות...");
    try {
      const response=await requestPage(nextPage); if(generation!==state.requestGeneration)return;
      const transactionIds=new Set(state.data.transactions.map((row)=>row.bank_transaction_id));
      const allocationIds=new Set(state.data.allocations.map((row)=>row.bank_allocation_id).filter(Boolean));
      const appended=(response.transactions||[]).filter((row)=>!transactionIds.has(row.bank_transaction_id));
      state.data.transactions.push(...appended);
      state.data.allocations.push(...(response.allocations||[]).filter((row)=>row.bank_allocation_id?!allocationIds.has(row.bank_allocation_id):appended.some((item)=>item.bank_transaction_id===row.bank_transaction_id)));
      state.data.pagination=response.pagination; state.page=response.pagination?.page||nextPage;
      appended.forEach((row)=>{const draft=readAutosaveDraft(`bank.allocations.${row.bank_transaction_id}`);if(draft)state.drafts.set(row.bank_transaction_id,draft);if(allocationsFor(row.bank_transaction_id).length>1)state.expanded.add(row.bank_transaction_id);});
      render(true);
    } catch(error) { message(error.message,"error"); }
    finally { if(generation===state.requestGeneration){state.loadingMore=false;$("#bank-scroll")?.setAttribute("aria-busy","false");setLoadingFeedback($("#bank-load-more-status"),false);} }
  };
  ...
  const clearFilter = (key) => {
    if(key.includes(":")){const [name,value]=key.split(":");state[name]=state[name].filter((item)=>item!==value);} else if(key==="query"){$("#bank-new-search").value="";state.query="";} else if(key==="description"){$("#bank-description-filter").value="";state.description="";} else if(key==="workflow")state.workflow="all"; else if(key==="batch")state.batch=""; else if(["split","descriptionPresence","referencePresence"].includes(key))state[key]=key==="split"?"any":""; state.page=1; reload();
  };
  ...
  $("#bank-file").addEventListener("change",async(event)=>{const file=event.target.files[0];if(!file)return;try{await withBusyControl($("#bank-import"),"מעבד קובץ...",async()=>{message("מעבד קובץ...");const data=await file.arrayBuffer(),parsed=await parseWorkbook(file,data,state.data.accounts);if(parsed.needsMapping){openMapping(file,data,parsed);message("");return;}message("בודק את הקובץ...");const preview=await request("POST",{action:"preview",account_number:parsed.accountNumber,rows:parsed.rows});openPreview(parsed,preview);message("");});}catch(error){message(error.message,"error");}finally{event.target.value="";}});
  ...
  $("#bank-workflow-filter").addEventListener("change",(event)=>{state.workflow=event.target.value;state.page=1;reload();});
  $("#bank-new-search").addEventListener("input",(event)=>{state.query=event.target.value;state.page=1;invalidatePendingLoads();clearTimeout(state.searchTimer);state.searchTimer=setTimeout(reload,250);});
  $("#bank-description-filter").addEventListener("input",(event)=>{state.description=event.target.value;state.page=1;invalidatePendingLoads();clearTimeout(state.searchTimer);state.searchTimer=setTimeout(reload,250);});
  ...
  $("#bank-scroll").addEventListener("scroll",()=>{const root=$("#bank-scroll");if(root.scrollHeight-root.scrollTop-root.clientHeight<240)loadMore();},{passive:true});
  $("#bank-clear-all").addEventListener("click",()=>{state.query=state.description=state.batch=state.descriptionPresence=state.referencePresence="";state.month=state.assignment=state.daycare=state.department=state.account=state.category=state.status=[];state.split="any";state.workflow="all";state.sort="date_desc";state.visualSortColumn="date";state.page=1;$("#bank-new-search").value="";$("#bank-description-filter").value="";reload();});
