...
export async function mountBankWorkbenchV2(request) {
  ...
  const renderFilterChips = () => {
    ...
    ["month","assignment","daycare","department","account","category","status"].forEach((key)=>state[key].forEach((value)=>chips.push([`${key}:${value}`,`${labels[key]}: ${value}`])));
  ...
  const render = (preserve = true) => {
    const scroll=preserve?captureScroll():null, rows=filtered(), pagination=state.data.pagination||{}; renderWorkflowCards(); renderFilterChips(); $("#bank-new-count").textContent=`מציג ${rows.length} מתוך ${pagination.total??rows.length} תנועות`;
