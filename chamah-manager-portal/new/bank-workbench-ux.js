...
export async function mountBankWorkbenchV2(request) {
  ...
  const reload = async () => {
    ...
    try {
      ...
      state.data={transactions:[],allocations:[],accounts:[],units:[],daycares:[],categories:[],accountingStatuses:[],assignmentMonths:[],calendarYears:[],batches:[],...response};
    ...
    } finally { if(generation===state.requestGeneration){scroll?.setAttribute("aria-busy","false");setLoadingFeedback($("#bank-load-more-status"),false);} }
  };
  const loadMore = async () => {
    const pagination=state.data?.pagination; if(state.loadingMore||!pagination?.hasNext)return;
    ...
    try {
      ...
      appended.forEach((row)=>{const draft=readAutosaveDraft(`bank.allocations.${row.bank_transaction_id}`);if(draft)state.drafts.set(row.bank_transaction_id,draft);if(allocationsFor(row.bank_transaction_id).length>1)state.expanded.add(row.bank_transaction_id);}); render(true);
