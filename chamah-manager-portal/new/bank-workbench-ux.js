...
export async function mountBankWorkbenchV2(request) {
  ...
  const reload = async () => {
    const params=new URLSearchParams({year:state.year,page:String(state.page),page_size:String(state.pageSize),query:state.query,description:state.description,queue:state.workflow,split:state.split,sort:state.sort,description_presence:state.descriptionPresence,reference_presence:state.referencePresence});
  ...
  $("#bank-description-filter").addEventListener("input",(event)=>{state.description=event.target.value;state.page=1;clearTimeout(state.searchTimer);state.searchTimer=setTimeout(reload,250);});
