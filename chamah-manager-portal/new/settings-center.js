...
export const SETTINGS_SECTIONS = [
  ...
  { id: 'organization', title: 'הארגון והמעונות', description: 'ישויות משפטיות, יחידות דיווח ומעונות.', tables: [
    { table: 'legal_entity_types', key: 'legal_entity_type_id', title: 'סוגי ישות משפטית', fields: [f('display_name','שם הסוג'),f('legal_entity_type_code','קוד קבוע','text',{technical:true}),f('display_order','סדר','number'),f('lifecycle_status','מצב','select',{options:statuses})] },
    { table: 'legal_entities', key: 'legal_entity_id', title: 'ישויות משפטיות', fields: [f('display_name','שם לתצוגה'),f('legal_name','שם משפטי'),link('legal_entity_type_id','סוג ישות','legal_entity_types','legal_entity_type_id'),f('registration_number','מספר רישום'),f('legal_entity_code','קוד קבוע','text',{technical:true}),f('display_order','סדר','number'),f('lifecycle_status','מצב','select',{options:statuses})] },
    { table: 'allocation_units', key: 'allocation_unit_id', title: 'יחידות ארגוניות', fields: [f('display_name','שם היחידה'),f('allocation_unit_type','סוג','select',{options:[option('DAYCARE','מעון'),option('OFFICE','משרד'),option('MANAGEMENT','הנהלה'),option('DEVELOPMENT','פיתוח'),option('OTHER','אחר')]}),link('legal_entity_id','ישות משפטית','legal_entities','legal_entity_id'),f('allocation_unit_code','קוד קבוע','text',{technical:true}),f('display_order','סדר','number'),f('lifecycle_status','מצב','select',{options:statuses})] },
    { table: 'daycares', key: 'daycare_id', title: 'מעונות', fields: [f('display_name','שם המעון'),link('legal_entity_id','ישות משפטית','legal_entities','legal_entity_id'),link('allocation_unit_id','יחידת דיווח','allocation_units','allocation_unit_id',{dependsOn:'legal_entity_id',match:'legal_entity_id'}),f('license_number','מספר רישיון'),f('address_text','כתובת'),f('opened_on','תאריך פתיחה','date'),f('closed_on','תאריך סגירה','date'),f('daycare_code','קוד קבוע','text',{technical:true}),f('display_order','סדר','number'),f('lifecycle_status','מצב','select',{options:statuses})] }
  ]},
  { id: 'classrooms', title: 'הפעלת מעונות וכיתות', description: 'הקשר בין שנה, מעון וכיתה.', tables: [
    { table: 'daycare_school_years', key: 'daycare_school_year_id', title: 'הפעלת מעון בשנה', fields: [link('school_year_id','שנת לימודים','school_years','school_year_id'),link('daycare_id','מעון','daycares','daycare_id'),f('is_operating','פעיל בשנה','boolean'),f('tuition_payment_count','גביית שכר לימוד','select',{options:[option(11,'11 תשלומים'),option(12,'12 תשלומים')]})] },
    { table: 'classrooms', key: 'classroom_id', title: 'כיתות', fields: [link('daycare_school_year_id','מעון ושנה','daycare_school_years','daycare_school_year_id',{display:'_label'}),f('display_name','שם הכיתה'),f('classroom_code','קוד קבוע','text',{technical:true}),f('is_mixed','כיתה מעורבת','boolean'),f('effective_from','בתוקף מתאריך','date'),f('effective_to','עד תאריך','date'),f('display_order','סדר','number'),f('lifecycle_status','מצב','select',{options:statuses.filter((item)=>item.value!=='ARCHIVED')})] },
  ...
  { id: 'finance', title: 'כספים והנהלת חשבונות', description: 'סעיפי תקציב, חשבונות בנק ומצבי טיפול.', tables: [
    ...
    { table: 'bank_accounts', key: 'bank_account_id', title: 'חשבונות בנק', fields: [f('display_name','שם החשבון'),link('legal_entity_id','ישות משפטית','legal_entities','legal_entity_id'),f('account_identifier_masked','מזהה חשבון מוסווה'),f('bank_account_code','קוד קבוע','text',{technical:true}),f('display_order','סדר','number'),f('lifecycle_status','מצב','select',{options:statuses})] },
    { table: 'accounting_statuses', key: 'accounting_status_id', title: 'מצבי הנהלת חשבונות', fields: [f('display_name','שם המצב'),f('accounting_status_code','קוד קבוע','text',{technical:true}),f('display_order','סדר','number'),f('is_final','מצב סופי','boolean'),f('lifecycle_status','מצב','select',{options:statuses})] }
  ]},
  { id: 'rules', title: 'צוות וכללים', description: 'תפקידים, הכשרות וכללי רישוי ותקינה.', tables: [
    { table: 'roles', key: 'role_id', title: 'תפקידים', fields: [f('display_name','שם התפקיד'),f('role_code','קוד קבוע','text',{technical:true}),f('role_group','קבוצת תפקיד'),f('daycare_relevant','רלוונטי למעון','boolean'),f('display_order','סדר','number'),f('lifecycle_status','מצב','select',{options:statuses})] },
    { table: 'certificate_types', key: 'certificate_type_id', title: 'תעודות והכשרות', fields: [f('display_name','שם התעודה'),f('certificate_type_code','קוד קבוע','text',{technical:true}),f('requires_expiry','דורש תאריך תפוגה','boolean'),f('display_order','סדר','number'),f('lifecycle_status','מצב','select',{options:statuses})] },
    { table: 'classroom_licensing_rules', key: 'classroom_licensing_rule_id', title: 'כללי רישוי כיתה', fields: [f('age_group','קבוצת גיל','select',{options:[option('INFANT','תינוקות'),option('TODDLER','פעוטות'),option('GRADUATE','בוגרים')]}),f('sqm_per_child','מ״ר לילד','number'),f('max_children','מקסימום ילדים','number'),f('rounding_method','שיטת עיגול','select',{options:[option('FLOOR_AFTER_TOTAL','עיגול כלפי מטה לאחר הסכימה')]}),f('valid_from','בתוקף מתאריך','date'),f('valid_to','עד תאריך','date'),f('lifecycle_status','מצב','select',{options:statuses})] },
...
const requiredFields = {
  ...
  accounting_statuses: ['display_name','accounting_status_code','display_order','lifecycle_status'],
...
export function mountSettingsCenter(root, request, { canEdit = false } = {}) {
  ...
  const bind = () => {
    ...
    root.querySelector('[data-settings-deactivate]')?.addEventListener('click',async()=>{if(!confirm('להשבית את ההגדרה? הרשומה וההיסטוריה יישמרו.'))return;const lifecycle=editing.config.fields.find((field)=>field.name==='lifecycle_status');const values=lifecycle?{lifecycle_status:lifecycle.options.some((item)=>item.value==='ARCHIVED')?'ARCHIVED':'INACTIVE'}:editing.config.fields.some((field)=>field.name==='is_operating')?{is_operating:false}:{is_selectable:false};try{const result=await request('PATCH',{table:editing.config.table,id:editing.id,values});data[editing.config.table]=data[editing.config.table].map((row)=>String(row[editing.config.key])===String(editing.id)?result.row:row);editing=null;message='ההגדרה הושבתה וההיסטוריה נשמרה.';hydrate();render();}catch(error){message=error.message;render();}});
  };
