...
export const SETTINGS_SECTIONS = [
  ...
  { id: 'finance', title: 'כספים והנהלת חשבונות', description: 'סעיפי תקציב, חשבונות בנק ומצבי טיפול.', tables: [
    { table: 'budget_categories', key: 'budget_category_id', title: 'סעיפי תקציב', fields: [f('display_name','שם הסעיף'),f('budget_category_code','קוד קבוע','text',{technical:true}),f('category_type','סוג','select',{options:[option('INCOME','הכנסה'),option('PAYROLL','שכר'),option('EXPENSE','הוצאה'),option('OTHER','אחר')]}),f('lifecycle_status','מצב','select',{options:statuses})] },
  ...
  { id: 'rules', title: 'צוות וכללים', description: 'תפקידים, הכשרות וכללי רישוי ותקינה.', tables: [
    ...
    { table: 'budget_rules', key: 'budget_rule_id', title: 'כללי תקציב', fields: [link('budget_category_id','סעיף תקציב','budget_categories','budget_category_id'),link('school_year_id','שנת לימודים','school_years','school_year_id'),f('rule_type','סוג כלל','select',{options:[option('TUITION_PER_CHILD','שכר לימוד לילד'),option('STAFFING_RATIO','יחס תקינה'),option('WORKING_DAYS','ימי עבודה'),option('FIXED_MONTHLY_AMOUNT','סכום חודשי קבוע'),option('PER_CHILD_AMOUNT','סכום לילד'),option('PERCENTAGE_OVERHEAD','אחוז תקורה'),option('MANUAL_EXCEPTION','חריגה ידנית')]}),link('daycare_id','מעון','daycares','daycare_id'),link('allocation_unit_id','יחידה ארגונית','allocation_units','allocation_unit_id'),link('age_group_id','קבוצת גיל','age_groups','age_group_id'),f('numeric_value','ערך מספרי','number'),f('text_value','ערך טקסטואלי'),f('effective_from','בתוקף מתאריך','date'),f('effective_to','עד תאריך','date'),f('lifecycle_status','מצב','select',{options:statuses})] },
...
const ruleHelp = {
  ...
  budget_rules: ['מגדיר כלל תקציבי מתוארך עבור סעיף ותחום ארגוני.','נדרש ערך מספרי או טקסטואלי; תחומי מעון, יחידה וקבוצת גיל מצמצמים את תחולת הכלל.','תלוי בסעיף תקציב ובמקורות התחום שנבחרו.','תקציב ודשבורדים פיננסיים; אינו משנה נתוני עבר נעולים.'],
...
function validate(config, row, data) {
  ...
  if (config.table === 'budget_rules' && (row.numeric_value === null || row.numeric_value === '') && !String(row.text_value || '').trim()) errors.push('יש להזין ערך מספרי או ערך טקסטואלי לכלל.');
