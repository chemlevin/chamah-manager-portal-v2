...
function fieldValue(field, row) {
  ...
  if (options) return options.find((option) => String(option.value) === String(value))?.label ?? value;
  return value ?? '—';
}
...
function inputTemplate(field, value, error, record) {
  ...
  else if (field.type === 'select') { const options = typeof field.options === 'function' ? field.options(record) : (field.options || []); input = `<select ${common} ${field.disableWhenEmpty && !options.length ? 'disabled' : ''}><option value="">${escapeHtml(field.placeholder || 'בחירה')}</option>${options.map((option) => `<option value="${escapeHtml(option.value)}" ${String(option.value) === String(value ?? '') ? 'selected' : ''}>${escapeHtml(option.label)}</option>`).join('')}</select>`; }
...
export function createAdministration({ root, metadata, repository, confirm = window.confirm.bind(window), onError = () => {} }) {
  ...
  const state = { status: 'loading', rows: [], query: '', filters: {}, sort: metadata.defaultSort || { field: metadata.primaryKey, direction: 'asc' }, page: 1, pageSize: metadata.pageSize || DEFAULT_PAGE_SIZE, editing: null, original: null, inspecting: null, errors: {}, saving: false, message: '' };
  const editableFields = metadata.fields.filter((field) => field.form !== false && field.name !== metadata.primaryKey);
  const standardFields = editableFields.filter((field) => !field.technical);
  const technicalFields = editableFields.filter((field) => field.technical);
  ...
  function render() {
    ...
    root.innerHTML = `<section class="admin-shell" dir="rtl" aria-label="${escapeHtml(metadata.pluralLabel || metadata.label)}">
      ...
      ${state.editing ? `<div class="admin-dialog-backdrop" data-admin-backdrop><section class="admin-dialog admin-dialog-wide" role="dialog" aria-modal="true" aria-labelledby="admin-form-title"><header><div><p class="admin-eyebrow">${state.original?.[metadata.primaryKey] == null ? 'רשומה חדשה' : 'עריכת רשומה'}</p><h2 id="admin-form-title">${escapeHtml(metadata.formTitle || metadata.label)}</h2></div><button type="button" data-admin-cancel aria-label="סגירה">×</button></header><form data-admin-form novalidate><div class="admin-form-grid">${standardFields.filter((field) => !field.visibleWhen || field.visibleWhen(state.editing)).map((field) => inputTemplate(field, state.editing[field.name] ?? field.defaultValue ?? '', state.errors[field.name], state.editing)).join('')}</div>${metadata.renderEditorExtensions ? `<div class="admin-editor-extensions">${metadata.renderEditorExtensions(state.editing, state.original)}</div>` : ''}${technicalFields.length ? `<details class="admin-technical"><summary>${escapeHtml(metadata.technicalLabel || 'פרטים טכניים מתקדמים')}</summary><p>${escapeHtml(metadata.technicalHelp || 'האזור מיועד לזיהוי יציב ולחיבור עתידי למקורות נתונים.')}</p><div class="admin-form-grid">${technicalFields.filter((field) => !field.visibleWhen || field.visibleWhen(state.editing)).map((field) => inputTemplate(field, state.editing[field.name] ?? field.defaultValue ?? '', state.errors[field.name], state.editing)).join('')}</div></details>` : ''}<footer><button class="admin-button secondary" type="button" data-admin-cancel>ביטול</button><button class="admin-button primary" type="submit" ${state.saving ? 'disabled' : ''}>${state.saving ? 'שומר…' : 'שמירה'}</button></footer></form></section></div>` : ''}
      ${state.inspecting ? `<div class="admin-dialog-backdrop" data-admin-inspector-backdrop><section class="admin-dialog admin-inspector" role="dialog" aria-modal="true" aria-labelledby="admin-inspector-title"><header><div><p class="admin-eyebrow">מפת תלות</p><h2 id="admin-inspector-title">איפה זה נמצא בשימוש?</h2></div><button type="button" data-admin-inspector-close aria-label="סגירה">×</button></header>${metadata.renderInspector(state.inspecting)}</section></div>` : ''}
    </section>`;
    bind();
  }
  ...
  function bind() {
    ...
    root.querySelectorAll('[data-admin-inspect]').forEach((button) => button.addEventListener('click', () => { state.inspecting = clone(state.rows.find((item) => String(item[metadata.primaryKey]) === button.dataset.adminInspect)); render(); }));
    ...
    root.querySelectorAll('[data-admin-inspector-close]').forEach((button) => button.addEventListener('click', () => { state.inspecting = null; render(); }));
    root.querySelector('[data-admin-inspector-backdrop]')?.addEventListener('click', (event) => { if (event.target === event.currentTarget) { state.inspecting = null; render(); } });
    ...
    root.querySelector('[data-admin-form]')?.addEventListener('change', (event) => { if (!event.target.name) return; const field = editableFields.find((item) => item.name === event.target.name); if (!field?.onChange) return; field.onChange(state.editing, state.editing[event.target.name]); render(); });
    ...
    if (state.editing) metadata.bindEditorExtensions?.(root, state.editing);
    if (state.inspecting) metadata.bindInspector?.(root, state.inspecting);
  }
