...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.GOOGLE_BUDGET_SHEET_ID || process.env.GOOGLE_SHEET_ID || DEFAULT_SHEET_ID,
      range: RANGE,
      majorDimension: 'ROWS',
    });
    const tables = parseBudgetTables(response.data.values || []);
    const budget = calculateBudgetModel(tables);
    sendJson(res, 200, { tables, budget });
  } catch (error) {
    console.error('Budget API error:', error);
    sendJson(res, 500, { error: ERROR_MESSAGE, code: 'BUDGET_SHEETS_LOAD_FAILED' });
  }
};
